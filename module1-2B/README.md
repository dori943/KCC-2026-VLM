...
### Run Module 2-A directly from image (vision only)
```bash
set OPENAI_API_KEY=YOUR_KEY
python -m app.cli run-module2a \
  --provider vision \
  --image path/to/input.png \
  --user-goal "Extract the coin from the narrow gap"
```

### Run Module 2-B directly from image (vision only)
```bash
set OPENAI_API_KEY=YOUR_KEY
...
### Export Module 2-B normalized context from image (vision only)
```bash
set OPENAI_API_KEY=YOUR_KEY
python -m app.cli export-module2b-normalized \
  --provider vision \
  --image path/to/input.png \
  --user-goal "Extract the coin from the narrow gap"
```
...
- Module 2-B reasoner itself is deterministic; optional external API usage is triggered only when `provider=vision` is selected.
