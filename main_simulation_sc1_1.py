...
from robot_controller_3_1 import PandaController, render_camera
...
LEFT_BASE_POSITION = [0.3, 0.4, 0.65]
RIGHT_BASE_POSITION = [0.9, 0.4, 0.65]
TABLE_BASE_POSITION = [0.6, 0.0, 0.0]
YCB_DIR = "/workspace/KCC-2026-VLM/data/object2urdf/examples/ycb"

YCB_OBJECT_SPECS = [
    ("cracker_box", "003_cracker_box.urdf", [0.1, -0.3, 0.82]),
    ("sugar_box", "004_sugar_box.urdf", [0.3, -0.3, 0.82]),
    ("pudding_box", "008_pudding_box.urdf", [0.5, -0.3, 0.82]),
    ("gelatin_box", "009_gelatin_box.urdf", [0.7, -0.3, 0.82]),
    ("bowl", "024_bowl.urdf", [0.9, -0.3, 0.82]),
    ("mug", "025_mug.urdf", [1.2, -0.3, 0.82]),
    ("plate", "029_plate.urdf", [0.2, 0.0, 0.82]),
    ("fork", "030_fork.urdf", [0.4, -0.15, 0.82]),
    ("spoon", "031_spoon.urdf", [0.6, -0.15, 0.82]),
    ("knife", "032_knife.urdf", [0.8, -0.15, 0.82]),
    ("spatula", "033_spatula.urdf", [1.1, -0.15, 0.82]),
    ("adjustable_wrench", "042_adjustable_wrench.urdf", [0.3, 0.1, 0.82]),
    ("large_marker", "040_large_marker.urdf", [0.5, 0.1, 0.82]),
    ("phillips_screwdriver", "043_phillips_screwdriver.urdf", [0.7, 0.1, 0.82]),
    ("flat_screwdriver", "044_flat_screwdriver.urdf", [0.9, 0.1, 0.82]),
]
...
def run_optional_affordance_probe(
    ...
    try:
        ...
        for label in labels_to_probe:
            ...
            matched = []
            generic = []
            semantic_subpart = []   # sub-part이지만 해당 물체에 속하는 것
            mismatched_parts = []
            for cand in candidates:
                ...
                if part_norm in _GENERIC_PART_TOKENS:
                    generic.append(cand)
                elif part_norm == target_norm:
                    matched.append(cand)
                else:
                    # crop이 이미 해당 물체 mask 기반으로 잘라낸 이미지이므로
                    # part 이름이 label과 달라도 해당 물체의 sub-part로 간주.
                    # 완전히 다른 물체 이름(예: "knife"가 "mug" crop에서 나옴)만
                    # 실제 mismatch이므로, 일단 semantic_subpart로 수용.
                    semantic_subpart.append(cand)
            ...
            if matched:
                ...
                if mismatched_parts:
                    print(
                        f"[R1][INFO] '{label}' part labels {mismatched_parts} "
                        "treated as sub-parts; using generic candidate."
                    )
            elif semantic_subpart:
                # crop이 해당 물체 전용이므로 sub-part 후보도 수용
                top = semantic_subpart[0]
                print(
                    f"[R1][INFO] '{label}' accepting semantic sub-part "
                    f"'{mismatched_parts[0]}' as valid grasp candidate."
                )
            else:
                print(f"[R1][WARN] '{label}' no candidates available after validation.")
                continue
...
def run_sequential_demo(
    ...
    # 수정 후
    _label_to_x = {lbl: pos[0] for lbl, _, pos in YCB_OBJECT_SPECS}

    _candidates = [lbl for lbl in _m3_labels if lbl in ycb_object_ids]
    if not _candidates:
        _candidates = _loaded_labels[:2]

    ARM_X_THRESHOLD = 0.6  # 이 값 이하 → left arm, 초과 → right arm

    if len(_candidates) >= 2:
        # x 기준 분류: x <= 0.6 → left, x > 0.6 → right
        _left_candidates  = [l for l in _candidates if _label_to_x.get(l, 0.0) <= ARM_X_THRESHOLD]
        _right_candidates = [l for l in _candidates if _label_to_x.get(l, 0.0) >  ARM_X_THRESHOLD]
        # 각 그룹에서 첫 번째 선택, 없으면 반대 그룹에서 fallback
        left_target_label  = _left_candidates[0]  if _left_candidates  else _candidates[0]
        right_target_label = _right_candidates[0] if _right_candidates else _candidates[-1]
